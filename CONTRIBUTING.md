...
Changes to technical-objective matching, language indexers, reachability, or framework relationships must update the labelled manifest under `testdata/technical-evaluation` when behavior intentionally changes. Label every candidate produced by a case, include hard negatives, and run `./scripts/evaluate-technical-evidence.sh`. Do not lower an acceptance threshold merely to make a regression pass; explain and review any threshold change independently.
