...
- Scan evidence bundles now use schema version 15. Version 15 adds `request_diagnostics` with one source-free timing/outcome record per repository-layer provider attempt; version 14 added checked cross-batch gap-resolution records, and version 13 added `provider_requests` and `source_batches_started`. Interrupted runs retain accounting and coverage but no unsynthesized semantic conclusions. These observations and unknowns do not create setup tasks, mutate profiles or the AI-use register, decide applicability, or affect CI gates.
- Repository-analysis prompt version 16 keeps source-batch candidates as evidence observations rather than durable AI-use identities and tightens compact-source response bounds. Independent source batches request sparse, directly evidenced records with lower reasoning effort where supported. Synthesis must assign every trusted observation exactly once and may resolve a batch-local question only with another member observation in the same group plus one of that observation's checked citations. Local validation rejects invented claims, self-resolution, unrelated groups, and citations outside the resolver. ComplyScan derives `inferred-use-*` IDs from sorted membership, removes only validated answered gaps, and reattaches the remaining facts, citations, objectives, unmapped observations, and genuinely unresolved questions without truncating their deterministic union. The private repository-analysis cache is schema/file 7 with context 13.
- Automatic model qualification identity now includes qualification contract version 2, repository-analysis prompt version 16, and setup-draft prompt version 6. Its private cache remains schema/file 4 at `model-qualification-v4.json`, so an older repository contract cannot authorize source transfer.
