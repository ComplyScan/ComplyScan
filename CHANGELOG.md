...
- Scan evidence bundles now use schema version 10. Each confirmed, draft, retired, or suggested AI use can contain nested `repository_facts` and `role_candidates`, while `ai_use_inventory.organization_unknowns` preserves contracts, actual use, operating regions, product control, market placement, and name-or-trademark questions that repository code cannot establish. These observations and unknowns are report-only: they do not create setup tasks, mutate profiles or the AI-use register, decide applicability, or affect CI gates. Changed-since scans retain full-repository deterministic fact coverage and label optional model facts with their bounded changed-plus-connected coverage.
- Repository-analysis prompt version 7 requires the typed positive-only per-use fact contract, reviewed-empty coverage, and exact AI-use bindings. Trusted validation checks supported fields and values, evidence paths and lines, confirmed-use citation scope, namespaced subsystem candidate identity, exact synthesis locations, duplicate fields, and forbidden negative or absence-based rationales. The private repository-analysis cache now uses schema/context version 3 so earlier results cannot bypass that contract.
- Automatic model qualification now includes repository-analysis prompt version 7 in its prompt-contract identity. Its private cache advances to schema 3 and `model-qualification-v3.json`, ensuring a model qualified only against an older repository response contract is checked again before receiving source context.
- Setup-draft prompt version 5 shares the same narrower repository-evident field vocabulary; deployment suggestions are limited to `embedded`, `api`, and `local-cli`, so repository prose cannot establish internal, customer-private, public, or open-source operation.
