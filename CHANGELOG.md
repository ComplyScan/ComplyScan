...
- Automatic model qualification identity now includes qualification contract version 3, repository-analysis prompt version 20, setup-draft prompt version 9, finding-review prompt version 3, and technical-review prompt version 12. Its private cache is schema/file 5 at `model-qualification-v5.json`, so an older inference contract cannot authorize source transfer.
- The standard cloud shortlist now pairs `gpt-5.6-terra`/`gpt-5.6-sol`, `claude-sonnet-5`/`claude-opus-5`, and `gemini-3.6-flash`/`gemini-3.7-flash`; every candidate remains explicitly quality-gates-pending until its current live benchmarks pass. Finding review, setup inference, source extraction, and search planning use low reasoning effort, while grouping synthesis and technical judgments use medium. OpenAI and Anthropic retain the sampling defaults required by their reasoning models, Gemini receives a fixed seed, and Ollama receives temperature zero. Setup-draft prompt version 9 compacts Anthropic's repeated fact-field union without weakening local field/value/evidence checks; the changed prompt identities prevent incompatible results from being reused.
