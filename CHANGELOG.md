...
- The default Markdown is now a one-page developer action plan: result, at most three prioritized actions with explicit do/why/where fields, a compact code-control table, the AI functionality found, repository-invisible questions, and scan coverage. Role hypotheses, complete fact tables, legal-source detail, and raw model/accounting records remain in schema-version 16 JSON for dashboards and audits instead of expanding `latest.md`. The model supplies structured cited observations, while local deterministic code controls evidence validation, report counts, priorities, labels, and layout.
