...
- The concise developer report now separates scan completion, technical safeguard evidence, legal applicability, and optional AI-use organization instead of collapsing them into one ambiguous verdict. It starts with at most three concrete actions, explains partial safeguards in plain language, distinguishes inferred workflows from shared infrastructure and evaluation tooling, and keeps detailed code-derived facts collapsible. Schema-version 16 JSON remains the exhaustive dashboard and integration contract and adds explicit grouping status plus per-attempt input/output/reasoning token counts; version 15 added privacy-bounded duration, outcome, and retry-reason diagnostics, and version 14 added checked `resolved_evidence_gaps`. The model supplies structured cited observations, while local deterministic code controls evidence validation, report counts, priorities, labels, and layout.
