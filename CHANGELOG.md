...
- Targeted hosted source requests now aim for roughly 24,000 input tokens, subject to lower configured or observed provider boundaries, while synthesis uses the configured model input allowance when combining validated summaries. The larger source packages retain the bounded queue but avoid many tiny extraction calls and keep more related evidence together. Synthesis output exhaustion grows through the adapter's safe model allowance before context is split, while raw-source responses retain the smaller 8,192-token recovery ceiling.
