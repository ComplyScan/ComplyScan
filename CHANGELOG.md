...
- Scan evidence bundles now use schema version 9 with a top-level `ai_use_inventory` overlay, per-confirmed-use `ai_use_mappings`, explicit `repository_analysis_run` lifecycle, and `repository_analysis.result.objective_observations[].ai_use_id` for a validated direct confirmed-use observation; this does not change deterministic findings or CI gating.
