...
- Repository-analysis prompt version 14 treats source-batch candidates as evidence observations rather than durable AI-use identities and introduces the grouping-only synthesis contract. Independent source batches now request sparse, directly evidenced records with lower reasoning effort where the selected adapter and model support it, instead of routine off-topic or merely uncertain objective rows. Synthesis must assign every trusted observation exactly once, may merge connected groups, cannot split a previously validated group, and returns no repeated evidence records. ComplyScan replaces temporary model grouping keys with `inferred-use-*` IDs derived locally from sorted membership, then reattaches every source batch's already-validated facts, citations, objective observations, unmapped observations, and questions; the deterministic union is not truncated to one response's eight-value fact limit. Source-request objective, system, citation, fact, and confirmed-use boundaries remain enforced. The private repository-analysis cache remains schema/file 6 and advances to context 9 for this assembly contract.
- Automatic model qualification identity now includes qualification contract version 2, repository-analysis prompt version 14, and setup-draft prompt version 6. Its private cache advances to schema/file 4 at `model-qualification-v4.json`, so older finding-only compatibility results cannot authorize source transfer.
