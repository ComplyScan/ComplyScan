...
- Repository-analysis prompt version 13 treats source-batch candidates as evidence observations rather than durable AI-use identities and introduces the grouping-only synthesis contract. Synthesis must assign every trusted observation exactly once, may merge connected groups, cannot split a previously validated group, and returns no repeated evidence records. ComplyScan replaces temporary model grouping keys with `inferred-use-*` IDs derived locally from sorted membership, then reattaches the source batches' already-validated facts, citations, objective observations, unmapped observations, and questions. Source-request objective, system, citation, fact, and confirmed-use boundaries remain enforced. The private repository-analysis cache remains schema/file 6 and advances to context 8 for this assembly contract.
- Automatic model qualification identity now includes qualification contract version 2, repository-analysis prompt version 13, and setup-draft prompt version 6. Its private cache advances to schema/file 4 at `model-qualification-v4.json`, so older finding-only compatibility results cannot authorize source transfer.
