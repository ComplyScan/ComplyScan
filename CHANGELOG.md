...
- Analysis setup now groups cloud services behind one `Hosted AI provider` choice and supports OpenAI, Anthropic, Google Gemini, xAI, Groq, Mistral, OpenRouter, and custom HTTPS OpenAI-compatible APIs. Hosted setup reads credentials only from the selected environment variable, loads the account's current model catalogue when available, retains exact-ID entry and offline suggestions as fallbacks, and runs the existing source-free compatibility check before repository context is sent.
