...
- Repository-analysis prompt version 8 requires the typed positive-only per-use fact contract, reviewed-empty coverage, and exact AI-use bindings. Trusted validation checks supported fields and values, evidence paths and lines, confirmed-use citation scope, namespaced subsystem candidate identity, exact synthesis locations, duplicate fields, and forbidden negative or absence-based rationales. Version 8 also replaces unsupported `oneOf` unions with equivalent disjoint `anyOf` variants for OpenAI Structured Outputs. The private repository-analysis cache uses schema 3; its context version is now 4 for the exhaustive targeted-queue contract.
- Automatic model qualification now includes repository-analysis prompt version 8 and setup-draft prompt version 6 in its prompt-contract identity. Its private cache remains schema 3 at `model-qualification-v3.json`; identity mismatch ensures that a model qualified against either older structured-output contract is checked again before receiving source context.
