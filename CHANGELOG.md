...
- The default Markdown is now a one-page developer action plan: result, at most three prioritized actions with explicit do/why/where fields, one framework technical assessment, the AI functionality found, repository-invisible questions, and scan coverage. The assessment names mapped provisions and counts code signals found, no matching signals, and checks that could not be completed; its key controls combine the framework/provision, code-level conclusion, checked evidence, and next step in one table rather than repeating a separate code-results section. These repository-evidence states are not legal applicability, pass/fail, or compliance scores. Role hypotheses, complete fact tables, legal-source detail, and raw model/accounting records remain in schema-version 17 JSON for dashboards and audits instead of expanding `latest.md`. The model supplies structured cited observations, while local deterministic code controls evidence validation, report counts, priorities, labels, and layout.
