...
- Local model setup now groups every choice as `Installed`, `Suggested download`, or `Custom model`, can search the live official Ollama catalogue and select a concrete local variant without knowing its tag, accepts manual exact tags as an offline fallback, and lists installed or catalogue-reported sizes in GB. Catalogue searches send only the search words to ollama.com, cloud-only variants are excluded from local mode, and every unseen model still has to pass the source-free compatibility check before repository context is sent.
