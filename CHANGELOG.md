...
- A normal `complyscan scan` now requires an approved AI provider and returns exit code 2 whenever that review cannot run or complete; `complyscan scan --deterministic-only` is the explicit model-free workflow. Missing or stale machine trust fails before discovery, while provider failures after discovery preserve the deterministic report. `--require-ai-review` remains a compatibility and CI policy flag and never grants consent. When deterministic selection finds no structural AI anchor but eligible code exists, targeted analysis sends a bounded representative manifest/runtime/configuration sample for an AI-presence check instead of silently treating the model layer as unnecessary. That expanded source boundary moves private consent to `adaptive-provider-pipeline-v6` and repository-analysis cache context to 15. The GitHub Action retains its safe `ai-review: none` default by explicitly passing deterministic-only mode.
- The default Markdown is now a short developer handoff: result, at most three developer actions with explicit do/done-when/why/evidence fields, demonstrated safeguards, AI functionality, product-or-compliance input, and one concise assessment-scope sentence. Framework context appears on the relevant action instead of in a repeated assessment table, while human-only context is separated and excluded from the developer-action count. Detailed framework coverage, action lifecycle IDs, and model/provider accounting remain in schema-version 17 JSON for dashboards and audits. Incomplete file or AI-review coverage remains a prominent warning so missing analysis cannot look clean. Related missing-evidence items are consolidated into concrete acceptance criteria, and conflicting model observations are described as uncertainty in the cited code rather than exposing source-batch pipeline terminology. These repository-evidence states are not legal applicability, pass/fail, or compliance scores. The model supplies structured cited observations, while local deterministic code controls evidence validation, report counts, priorities, labels, and layout.
...
- Default `auto` and `targeted` repository analysis queue one bounded excerpt from every structural candidate file instead of treating a per-request target as a repository-wide evidence cap. Small queues still use one request; larger queues use bounded source batches and synthesis, and changed-since scans exhaust candidates only inside their changed-plus-connected model boundary. If no structural anchor exists, a bounded representative presence sample now covers eligible manifests, likely runtime source, and configuration while de-prioritising tests. A completed review may report zero model-identified AI uses only after every selected batch validates. If all source batches validate but global grouping fails, every checked observation remains separately actionable and only grouping is marked incomplete; incomplete source reviews retain accounting but no unvalidated conclusions. Per-batch persistence and resume are not implemented yet. The private repository-analysis cache uses schema/file version 8 and context version 15, stored as `repository-analysis-v8.json`, and caches only results with completed or unnecessary grouping. The technical-review cache uses schema/file version 3 at `technical-review-v3.json`; both bind an available qualified model-artifact digest so an updated Ollama tag cannot reuse an older model's conclusions. Machine-local consent is bound to context contract `adaptive-provider-pipeline-v6` because the no-anchor source scope expanded.
...
- Scan evidence bundles now use schema version 17. Version 17 adds `developer_actions` and scan-input provenance; version 16 added `grouping_status` plus exact per-attempt input/output/reasoning token counts, version 15 added duration/outcome/retry request diagnostics, version 14 added checked cross-batch gap-resolution records, and version 13 added `provider_requests` and `source_batches_started`. Source-complete/grouping-incomplete runs retain validated observations, while incomplete source runs retain accounting and coverage but no unvalidated semantic conclusions. These observations and unknowns do not create setup tasks, mutate profiles or the AI-use register, decide applicability, or affect CI gates.
- Repository-analysis prompt version 20 retains version 17's atomic block-cited observations and membership-only synthesis while binding a stage-specific inference policy: source extraction uses low reasoning effort and grouping uses medium. Anthropic receives a compact equivalent wire grammar for repeated block and fact variants while ComplyScan retains the complete strict schema for local validation. Source responses cite trusted request-local block IDs and original lines; ComplyScan assigns observation identity and unions checked nested fact citations locally. One bounded corrective regeneration remains for genuinely malformed output before smaller splitting. Local validation rejects invented claims, self-resolution, unrelated groups, and citations outside the resolver. ComplyScan derives `inferred-use-*` IDs from sorted membership and reattaches the complete checked records. The private repository-analysis cache is schema/file 8 with context 15.
- Automatic model qualification identity now includes qualification contract version 3, repository-analysis prompt version 20, setup-draft prompt version 10, finding-review prompt version 3, and technical-review prompt version 12. Its private cache is schema/file 5 at `model-qualification-v5.json`, so an older inference contract cannot authorize source transfer.
- The standard cloud shortlist now pairs `gpt-5.6-terra`/`gpt-5.6-sol`, `claude-sonnet-5`/`claude-opus-5`, and `gemini-3.6-flash`/`gemini-3.7-flash`; every candidate remains explicitly quality-gates-pending until its current live benchmarks pass. Finding review, setup inference, source extraction, and search planning use low reasoning effort, while grouping synthesis and technical judgments use medium. OpenAI and Anthropic retain the sampling defaults required by their reasoning models, Gemini receives a fixed seed, and Ollama receives temperature zero. Setup-draft prompt version 10 compacts Anthropic's repeated fact-field union and constrains evidence paths to the submitted request without weakening local field/value/evidence checks; the changed prompt identities prevent incompatible results from being reused.
...
- `complyscan scan` is the single normal workflow: it always runs deterministic checks and requires the saved advisory provider backed by `ai.review-on-scan: true` plus matching private trust recorded by guided setup on that machine. The trust identity binds provider, endpoint, model, and credential environment-variable name, so a clone or configuration change cannot silently authorize source transfer. Provider/model/endpoint/refresh/deep/review options remain explicit one-run consent; `--deterministic-only` is the explicit no-model workflow. Provider failures preserve the deterministic report but make the standard scan incomplete. `--require-ai-review` remains policy only and never grants consent; the hidden `complyscan review` command remains a migration compatibility path.
