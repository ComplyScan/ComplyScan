...
- The concise developer report now separates scan completion, technical safeguard evidence, and legal applicability instead of collapsing them into one ambiguous verdict. It starts with at most three concrete actions, explains partial safeguards in plain language, distinguishes inferred workflows from shared infrastructure and evaluation tooling, and keeps detailed code-derived facts collapsible. Schema-version 14 JSON remains the exhaustive dashboard and integration contract and adds checked `resolved_evidence_gaps`, distinguishing source-batch questions answered during global synthesis from genuinely unresolved technical questions and report-only organisation unknowns. The model supplies structured cited observations, while local deterministic code controls report counts, priorities, labels, and layout.
...
- Targeted synthesis now keeps the roughly 6,500-token boundary for raw source-excerpt batches but uses the configured model input allowance when combining their validated summaries. Synthesis output exhaustion grows through the adapter's safe model allowance before context is split, while raw-source responses retain the smaller 8,192-token recovery ceiling. This prevents validated summaries from being fragmented into dozens of same-level calls or repeatedly split and recombined; 13-batch regressions now converge in one global synthesis request, or one unchanged 4K→8K→16K recovery sequence, when the model context permits it.
...
- Default `auto` and `targeted` repository analysis now queue one bounded excerpt from every structural candidate file instead of treating a roughly 6,500-token request target as a repository-wide evidence cap. Small queues still use one request; larger queues use bounded source batches and synthesis, and changed-since scans exhaust candidates only inside their changed-plus-connected model boundary. A completed review may report zero model-identified AI uses only after every candidate batch validates. Interrupted reviews preserve provider-request, attempted-transfer, and source-batches-started-versus-validated coverage but discard unsynthesized model conclusions; per-batch persistence and resume are not implemented yet. The private repository-analysis cache now uses schema/file version 6 and context version 10, stored as `repository-analysis-v6.json`, and the technical-review cache uses schema/file version 3 at `technical-review-v3.json`; both bind an available qualified model-artifact digest so an updated Ollama tag cannot reuse an older model's conclusions. Machine-local consent now binds context contract `adaptive-provider-pipeline-v2`; rerun setup to grant reusable approval or use explicit one-run provider flags.
- Independent source batches, output-exhausted members of the same completed source wave, and—after source validation—independent groups within each synthesis level now use one provider-neutral adaptive scheduler. When both request-per-minute and token-per-minute response headers are available, both dimensions bound each concurrent wave; partial capacity remains sequential. Header-free hosted runs start at one request and ramp conservatively after successful waves, while a local ceiling of 32 prevents unbounded socket concurrency; Ollama remains serial because it exposes no portable capacity metadata. A fresh source-free capacity probe may seed a cached multi-batch run before repository code is transferred, but is itself metered and counted. Provider-wide validation repairs, transient failures, and rate limits use jittered bounded retries through the same gate; permanent quota failures return promptly, cumulative automatic wait is capped at ten minutes per logical request, transient retries at eight cycles, and repository analysis at 256 total provider requests. These bounds preserve deterministic output but cannot guarantee a completed AI result under permanent provider or schema failure.
- Live source-free model qualification now exercises both finding-binding and repository-shaped structured contracts. The two phases normally use two requests and share a four-request ceiling for typed temporary retries; permanent quota, request-size, cancellation, and contract failures stop promptly. Finding review, technical-search planning, and technical evidence judgments use the same bounded provider-neutral taxonomy; metered failed attempts and validated partial observations are preserved, and required AI review now fails honestly when requested findings or technical targets remain unreviewed. Scan, setup, and doctor output retain exact request and token accounting. Native Gemini keeps trusted system instructions separate from untrusted repository input; Anthropic context overflow is adaptively split, and Anthropic/OpenAI-compatible reasoning-token details are retained where providers report them.
- Scan evidence bundles now use schema version 13. Version 13 adds `provider_requests` and `source_batches_started`; `source_batches_completed` counts validated batches rather than all started work. Version 12's exact `member_observation_ids`, version 11's completed/total source-batch counters, and version 10's nested `repository_facts`, `role_candidates`, and report-only organisation unknowns remain available. Interrupted runs retain accounting and coverage but no unsynthesized semantic conclusions. These observations and unknowns do not create setup tasks, mutate profiles or the AI-use register, decide applicability, or affect CI gates.
- Repository-analysis prompt version 15 treats source-batch candidates as evidence observations rather than durable AI-use identities and extends compact grouping synthesis with checked cross-batch gap resolution. Independent source batches request sparse, directly evidenced records with lower reasoning effort where supported. Synthesis must assign every trusted observation exactly once and may resolve a batch-local question only with another member observation in the same group plus one of that observation's checked citations. Local validation rejects invented claims, self-resolution, unrelated groups, and citations outside the resolver. ComplyScan derives `inferred-use-*` IDs from sorted membership, removes only validated answered gaps, and reattaches the remaining facts, citations, objectives, unmapped observations, and genuinely unresolved questions without truncating their deterministic union. The private repository-analysis cache remains schema/file 6 and advances to context 10.
- Automatic model qualification identity now includes qualification contract version 2, repository-analysis prompt version 15, and setup-draft prompt version 6. Its private cache remains schema/file 4 at `model-qualification-v4.json`, so an older repository contract cannot authorize source transfer.
