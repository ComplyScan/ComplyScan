...
- Repository-analysis prompt version 12 treats source-batch candidates as evidence observations rather than durable AI-use identities, supplies trusted validation feedback for finite structured-output repair, and hardens provider-neutral synthesis. Synthesis must assign every trusted observation exactly once, may merge connected groups but cannot split a previously validated group, and may cite only exact member-scoped locations while preserving positive fact values and definite objective evidence. ComplyScan replaces temporary model grouping keys with `inferred-use-*` IDs derived locally from sorted observation membership. Request-bound objective, system, and confirmed-use enums remain enforced. The private repository-analysis cache advances to schema/file 6 and context 7 for this result and model-artifact identity contract.
- Automatic model qualification identity now includes qualification contract version 2, repository-analysis prompt version 12, and setup-draft prompt version 6. Its private cache advances to schema/file 4 at `model-qualification-v4.json`, so older finding-only compatibility results cannot authorize source transfer.
