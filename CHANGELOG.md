...
- Targeted synthesis now keeps the roughly 6,500-token boundary for raw source-excerpt batches but uses the configured model input allowance when combining their validated summaries. This prevents one oversized summary from being fragmented into dozens of same-level synthesis calls; a 13-batch regression now converges in one global synthesis request when the summaries fit the configured model context.
