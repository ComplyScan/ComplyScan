...
- Default `auto` and `targeted` repository analysis now queue one bounded excerpt from every structural candidate file instead of treating a roughly 6,500-token request target as a repository-wide evidence cap. Small queues still use one request; larger queues use bounded source batches and synthesis, and changed-since scans exhaust candidates only inside their changed-plus-connected model boundary. A completed review may report zero model-identified AI uses only after every candidate batch completes. Interrupted reviews preserve attempted-transfer and completed-versus-total batch coverage but discard unsynthesized model conclusions; per-batch persistence and resume are not implemented yet. The private repository-analysis cache now uses schema 4 and context version 5 so earlier single-package and pre-membership results cannot be reused. The machine-local consent digest also includes the exhaustive context contract, so prior one-package approval does not silently authorize multiple requests; rerun setup to grant reusable approval or use explicit one-run provider flags.
- Native OpenAI source batches now use the effective request/token remaining and reset headers returned by the API to run independent batches concurrently in bounded waves. A live source-free compatibility response can seed the complete first wave; when compatibility is cached and multiple source batches are needed, one fresh source-free capacity probe runs before repository code is transferred. Only a failed or header-free probe calibrates with the first bounded source request instead. Missing headers stay sequential, exhausted capacity waits for reset, unexpected `429` responses keep bounded retry accounting, and a local ceiling of 32 prevents an RPM allowance from becoming unbounded socket concurrency. Synthesis still starts only after every source batch completes.
- Scan evidence bundles now use schema version 12. Version 12 adds `member_observation_ids` to inferred repository AI uses so dashboards and audits can see the exact batch observations grouped under a locally assigned candidate ID. Version 11's completed/total source-batch counters and version 10's nested `repository_facts`, `role_candidates`, and report-only organisation unknowns remain available. These observations and unknowns do not create setup tasks, mutate profiles or the AI-use register, decide applicability, or affect CI gates.
- Repository-analysis prompt version 10 treats source-batch candidates as evidence observations rather than durable AI-use identities. Synthesis must assign every trusted observation exactly once, may merge connected groups but cannot split a previously validated group, and may cite only exact locations belonging to the selected members. ComplyScan replaces temporary model grouping keys with `inferred-use-*` IDs derived locally from sorted observation membership. Request-bound objective, system, and confirmed-use enums from version 9 remain enforced. The private repository-analysis cache advances to schema 4/context 5 for this result contract.
- Automatic model qualification now includes repository-analysis prompt version 10 and setup-draft prompt version 6 in its prompt-contract identity. Its private cache remains schema 3 at `model-qualification-v3.json`; identity mismatch ensures that a model qualified against either older structured-output contract is checked again before receiving source context.
