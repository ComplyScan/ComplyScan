...
- Default `auto` and `targeted` repository analysis now queue one bounded excerpt from every structural candidate file instead of treating a roughly 6,500-token request target as a repository-wide evidence cap. Small queues still use one request; larger queues use bounded source batches and synthesis, and changed-since scans exhaust candidates only inside their changed-plus-connected model boundary. A completed review may report zero model-identified AI uses only after every candidate batch completes. Interrupted reviews preserve attempted-transfer and completed-versus-total batch coverage but discard unsynthesized model conclusions; per-batch persistence and resume are not implemented yet. The private repository-analysis cache remains schema 3 and advances to context version 4 so earlier single-package results cannot be reused. The machine-local consent digest also includes the new exhaustive context contract, so prior one-package approval does not silently authorize multiple requests; rerun setup to grant reusable approval or use explicit one-run provider flags.
- Scan evidence bundles now use schema version 11. Version 11 adds explicit completed and total source-batch counters for complete and interrupted repository reviews. Each confirmed, draft, retired, or suggested AI use can retain the version-10 nested `repository_facts` and `role_candidates`, while `ai_use_inventory.organization_unknowns` preserves contracts, actual use, operating regions, product control, market placement, and name-or-trademark questions that repository code cannot establish. These observations and unknowns are report-only: they do not create setup tasks, mutate profiles or the AI-use register, decide applicability, or affect CI gates. Changed-since scans retain full-repository deterministic fact coverage and label optional model facts with their bounded changed-plus-connected coverage.
- Repository-analysis prompt version 9 binds objective, system, and confirmed-AI-use observation IDs to request-specific structured-output enums; when no confirmed use is supplied, `objective_observations.ai_use_id` can only be empty. This prevents repository names or model-authored candidate IDs from being misclassified as trusted durable IDs and aborting an otherwise valid batch. It retains version 8's typed positive-only per-use fact contract, reviewed-empty coverage, portable disjoint `anyOf` unions, namespaced subsystem candidate identity, and exact synthesis locations. The private repository-analysis cache uses schema 3; its context version remains 4 for the exhaustive targeted-queue contract.
- Automatic model qualification now includes repository-analysis prompt version 9 and setup-draft prompt version 6 in its prompt-contract identity. Its private cache remains schema 3 at `model-qualification-v3.json`; identity mismatch ensures that a model qualified against either older structured-output contract is checked again before receiving source context.
