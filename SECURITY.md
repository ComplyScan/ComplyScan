...
- a trusted or explicitly consented scan runs the deterministic foundation first and then sends one or more bounded, best-effort-redacted candidate packages, followed by synthesis when needed, to the configured loopback or hosted provider. Independent native OpenAI source batches may run concurrently within observed request/token allowances, so variable hosted cost can be incurred faster. Hosted processing remains subject to that provider's terms and account settings. A missing key, model, or provider leaves deterministic results intact and marks the AI layer incomplete.
