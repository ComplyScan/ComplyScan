...
var setupQuestionHelp = map[string][]string{
	...
	"remote-disclosure": {
		"Later cloud-assisted `complyscan scan` runs send every structurally selected candidate file as one or more bounded source requests after recognised-secret redaction, followed by bounded synthesis when multiple requests are needed. Independent source batches may run concurrently within the provider's observed request and token allowances. Setup itself does not send repository source. Selection uses local inventory signals, technical-objective matches, production entry points, and their bounded code-graph neighborhood. Ignored, generated, dependency, binary, oversized, excluded, and non-candidate files remain outside this context.",
