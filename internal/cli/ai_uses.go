...
func loadAIUseReport(path string) (reportpkg.Report, error) {
	...
	if value.SchemaVersion != 6 && value.SchemaVersion != 7 && value.SchemaVersion != 8 && value.SchemaVersion != 9 && value.SchemaVersion != 10 && value.SchemaVersion != 11 && value.SchemaVersion != 12 {
		return reportpkg.Report{}, fmt.Errorf("ComplyScan report %q uses unsupported schema version %d (want 6, 7, 8, 9, 10, 11, or 12)", path, value.SchemaVersion)
	}
