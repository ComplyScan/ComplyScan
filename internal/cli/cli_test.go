...
func TestScanJSONOutputAndSeverityFilter(t *testing.T) {
	...
	if decoded.SchemaVersion != 17 || decoded.Tool.Commit != "test" || decoded.Scan.ID == "" || decoded.Scan.Scope.Findings != "full-repository" || decoded.Scan.Scope.TechnicalEvidence != "full-repository" {
...
func TestScanMapsConfirmedAIUseToSystemRequirementsAndScopedEvidence(t *testing.T) {
	...
	if decoded.SchemaVersion != 17 || decoded.AIUseMappings == nil || len(decoded.AIUseMappings.Uses) != 1 {
...
func TestScanMapsSharedEvidenceAcrossEUAndNISTFrameworks(t *testing.T) {
	...
	if decoded.SchemaVersion != 17 || len(decoded.Frameworks) != 2 {
