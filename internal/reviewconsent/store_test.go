...
func TestConsentInvalidatesImmediatePriorAdaptiveProviderContextContract(t *testing.T) {
	...
	if reviewContextContractVersion != "adaptive-provider-pipeline-v6" {
		t.Fatalf("current review context contract = %q, want adaptive-provider-pipeline-v6", reviewContextContractVersion)
	}
	...
	prior := current
	prior.ContextContract = "adaptive-provider-pipeline-v5"
