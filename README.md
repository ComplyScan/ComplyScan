...
> Current release: v0.1.7. Model review is optional and experimental; deterministic scanning remains the default.
...
Running `complyscan` is the recommended first command. With no configuration it uses a four-step setup: inspect the repository locally without a model; choose local AI, an explicitly consented hosted provider, or model-free analysis; choose the technical mappings to run; then review the setup while deciding whether to save and run a quick scan, save and run a deep scan, or save without scanning. The default path does not ask the business and legal applicability questionnaire. It creates an explicitly unreviewed internal report target, named from the repository, so code evidence has a system to attach to in reports. This record is not a legal profile: it records only conservative code-visible AI activity and leaves jurisdiction, organisation role, intended purpose, deployment, legal applicability, and legal risk class unknown. Selecting the EU technical pack means “scan this code against these technical objectives,” not “the EU AI Act legally applies.” Use `complyscan profile setup --replace` later to complete one profile, or `complyscan setup --advanced` for the full questionnaire and attributed human applicability decision. Terminal setup saves private recovery checkpoints between major stages and offers to resume them after interruption; these drafts contain configuration answers but no repository source or API-key value and are removed after `.complyscan.yml` is saved.

In an interactive terminal, fixed single-answer questions use an arrow-controlled radio menu, multiple-answer questions use checkboxes, and confirmations show a highlighted Yes/No choice. Move with ↑/↓, tick or untick with Space where applicable, press Enter to confirm, and press ← to revisit the preceding question without losing completed answers. Text and accessible prompts accept `back` because they do not receive interactive arrow-key events. `Ctrl+C` still cancels setup. Local and remote model pickers use the same menus and include an explicit custom-model option before asking for free text. These controls also cover the system, test-command, and objective choices in verification setup. Redirected setup scripts keep the stable numbered or text interface. Set `COMPLYSCAN_ACCESSIBLE=1` to force that interface for screen readers or terminals where cursor-based controls are unsuitable.

The optional detailed profile flow ends with an applicability-readiness gate. `incomplete` means one or more facts used by the current technical mapping are still unknown; `factually-ready` means those inputs are present but have no named factual reviewer; `human-reviewed` means the factual profile has a named reviewer. These statuses describe input quality only. They do not decide that the EU AI Act applies and do not replace the separate legal applicability decision, which remains `needs-review` unless an accountable human records otherwise. The short setup intentionally starts with an incomplete profile; that never prevents repository discovery or technical-objective scanning, and legal requirement mapping remains provisional.
...
Interactive setup selects `qwen3.5:9b` as the recommended local default candidate, lists other installed and suggested models, and accepts any Ollama tag. Every ready selection receives an automatic synthetic compatibility check, cached for 30 days and invalidated by provider, exact model or available Ollama digest, and prompt-contract changes. This status does not transfer a maintained benchmark claim between models. Automation never installs software or downloads a model unless `--install-ollama` or `--pull-model` is explicitly passed. Non-interactive remote setup additionally requires `--allow-remote-review`, and contacts the model only when `--qualify-model` is supplied; use `--non-interactive --review none` for a network-free starter configuration. See [automatic model qualification](docs/model-qualification.md).
...
The built-in scan default remains deterministic. Interactive `complyscan setup` first inspects repository structure and technical signals locally without a model, then offers local Ollama, a hosted provider, or model-free analysis. Installation remains behind separate confirmation. The selected model receives or reuses a source-free automatic compatibility check before any later deep scan may send repository context. The short setup makes no repository-context model request until the user confirms “Save and run deep AI-assisted scan” on the final review screen. The picker lists models already installed by Ollama with their reported size in GB, labels `qwen3.5:9b` as the recommended default with an onboarding benchmark, identifies `qwen3:8b` as having a recorded technical-review baseline, shows approximate GB download sizes for its recommendation set, and accepts any exact Ollama tag. Manual setup remains available:
...
The guided equivalent is `complyscan setup`. Its first menu contains Local AI, Hosted AI provider, and model-free technical analysis. The hosted submenu offers OpenAI, Anthropic, Google Gemini, xAI, Mistral, GroqCloud, OpenRouter, and another OpenAI-compatible API. GroqCloud is labelled as a fast hosted catalogue containing models from several model makers and appears directly above OpenRouter. After consent and selection of the API-key environment variable, setup requests the models currently available to that account and presents a searchable selector. Exact model-ID entry and provider-specific starting suggestions remain available when discovery is unsupported or unavailable. Non-interactive setup must explicitly include `--allow-remote-review`. Run `complyscan doctor --probe-review` for a small live structured-output check before scanning sensitive repositories.
...
The experimental default candidate is the official Ollama `qwen3.5:9b` model. `qwen3:8b` remains selectable with its recorded technical-review baseline, and the official `qwen3-coder:30b` is available as a substantially larger option. Any other exact tag may be selected and automatically qualified without maintainer approval. Setup may install Ollama, start its Homebrew service, or pull the selected model only after explicit confirmation; normal scans never install software or models. Fake-transport tests enforce the structured-output, redaction, injection, system/objective binding, grounded-path, bounded-retrieval, and isolated-test interpretation contracts. The recorded prompt-version 10 fixture gate and timing results were produced with `qwen3:8b`; they do not transfer automatically to another compatible model. Broader representative quality and performance evaluation, plus the maintained applicability reassessment, remain open before Ollama investigation can leave experimental status.
...
`complyscan setup` is a separate provisioning boundary. After explicit confirmations it may invoke Homebrew, download and run Ollama's official Linux installer, start the Homebrew Ollama service, and use `ollama pull` to download the selected model. Default setup does not send repository content to the selected model; repository context is sent only by an explicitly selected deep scan after setup. Non-interactive setup performs no installation or model download unless the corresponding install or pull flag is supplied.
...
Ollama, OpenAI, Anthropic, Gemini, xAI, Groq, Mistral, OpenRouter, and custom OpenAI-compatible APIs are implemented review providers. The built-in scan default remains `none`; interactive setup presents the local/hosted/model-free privacy boundary before selecting a provider. Every hosted provider requires an explicit external-processing confirmation, an environment-only API key, and a user-selected model before any profile-draft context is submitted.
