...
Use-scoped mappings are advisory and do not change deterministic findings or CI thresholds. When the configured AI layer runs, ComplyScan derives a typed context for each active confirmed use that has files inside the current model boundary. It contains the stable human-owned ID, description and path scope, associated configured systems, the exact files selected for that use, and only objectives currently screened as likely required or recommended. The raw register is not sent. The model evaluates these use/objective/system combinations separately inside every applicable bounded source request, and synthesis must preserve one direct observation for each combination with the supplied stable AI-use ID. Definite decisions require checked citations from that use's submitted files; an uncited result can only say that the bounded package is insufficient to decide. Trusted validation rejects omitted or unknown combinations and out-of-scope citations.
...
JSON and SARIF 2.1.0 output remain buffered so they are valid and deterministically ordered. Schema-version 12 JSON stdout and `latest.json` include applicability, the raw technical AI inventory, the `ai_use_inventory` overlay, per-use `ai_use_mappings`, technical evidence, reconciliation, the explicit `repository_analysis_run` lifecycle, completed/total source-batch coverage, and exact `member_observation_ids` for every inferred repository AI use. Version 11 introduced the batch counters and version 10 introduced `repository_facts`, `role_candidates`, and report-level organisation unknowns. Version 9's optional direct confirmed-use observation binding remains available. SARIF includes source locations and stable partial fingerprints for code-scanning integrations, but omits technical-objective summaries that have no single code-scanning location.
...
When `ai.review-on-scan` is backed by matching private machine trust, or a provider is supplied with explicit one-run options, `complyscan scan` runs targeted repository AI reasoning after deterministic discovery, secret redaction, file classification, inventory, technical-evidence matching, and graph construction. Local code ranks AI signals, objective matches, production entry points, imports, callers, and nearby graph relationships, then prepares one bounded excerpt for every structurally selected candidate file. A small queue can fit one request; a larger queue is partitioned into provider-sized source batches and synthesized after every batch completes. The normal remote target remains about 6,500 input tokens per request, not per repository. A single-package review may request one bounded follow-up with at most three literal searches; trusted code retrieves at most three new excerpts and, only when useful evidence is found, makes one final request. `--deterministic-only` never enters this path.

For native OpenAI, ComplyScan reads the effective request/token limit, remaining-capacity, and reset headers returned by the API. There is no separate ordinary API-key endpoint that reports the current tier before a request. A live source-free compatibility response can seed the first source-batch wave. When compatibility was reused from the private cache and a multi-batch review is actually needed, ComplyScan makes one fresh source-free capacity probe before sending code; if that probe returns no usable headers, the first bounded source batch calibrates current capacity instead. Remaining independent source batches then run concurrently up to the smallest safe request allowance, conservative estimated token allowance, pending batch count, and a local concurrency ceiling of 32. Exhausted capacity waits for the disclosed reset before the next wave, and temporary `429` responses retain the existing bounded retry behavior. Missing or unsupported headers preserve sequential execution. Global synthesis starts only after every source batch completes, so scheduling does not change selected evidence or grouping semantics. See the [OpenAI API rate-limit response headers](https://developers.openai.com/api/reference/overview#backwards-compatibility).

The exhaustive request pattern is part of the private consent fingerprint. Machine approval created for the former one-package behavior does not authorize it automatically; rerun `complyscan setup` to review the multiple-request and variable-cost disclosure, or use explicit one-run provider flags for that invocation.
