...
When Ollama is explicitly enabled, ComplyScan performs a separate technical-objective review after deterministic matching. It sends one existing candidate at a time with bounded graph relationships and small connected symbol excerpts. The model receives no evidence fingerprint and returns no binding identifiers; trusted ComplyScan code attaches the sole decision to the submitted objective and fingerprint. Returned strengths (`strong`, `partial`, `weak`, `uncertain`, or `not_supported`) cannot create an objective, change its status, decide legal applicability, or affect the exit code.
...
Technical observations are cached in the operating system's private user-cache directory, not in the scanned repository. The cache stores the same bounded, redacted observation used in reports plus hashes of candidate context; it does not store the submitted source-context records. Model rationales are instructed not to quote code, but may still describe repository details and should be treated as potentially sensitive local data. Reuse requires the same provider, model tag, prompt version, control-pack ID/version/digest, objective, evidence fingerprint, and complete bounded candidate-context digest. A code, context, pack, prompt, or model-name change therefore triggers a new request. Use `--refresh-review` to deliberately bypass existing technical observations. Each scan prints whether a candidate is being reviewed by Ollama or reused from cache; completed model responses are saved individually so an interrupted long scan can resume.

The experimental default is the official Ollama `qwen3:8b` model. Ollama does not currently publish a `qwen3-coder:8b` tag; its official Qwen3-Coder family starts at `30b`, which has a substantially larger installation footprint. The model remains configurable. Setup may install Ollama, start its Homebrew service, or pull the selected model only after explicit confirmation; normal scans never install software or models. Fake-transport tests enforce the structured-output, redaction, injection, and identifier-binding contract. Three fresh repeated public-repository runs now pass the effective quality gate consistently, but raw-model performance remains materially dependent on transparent deterministic guardrails; broader representative evaluation and the maintained applicability reassessment remain open before Ollama review can leave experimental status.

ComplyScan uses two separate review flows, both bounded by `max-findings`. The finding flow sends visible, unsuppressed deterministic finding records with re-redacted metadata such as the rule ID, fingerprint, title, message, relative path, line, short evidence, remediation, severity, and confidence. The technical flow makes one request per existing candidate and sends its objective ID, reachability, imports, graph relationships, unresolved questions, a bounded window around the match, and up to six connected source-symbol excerpts. It deliberately withholds the evidence fingerprint. ComplyScan never sends a complete repository or an unbounded file. Input excerpts are re-redacted and are not copied into the saved report. Reviewing many technical candidates can therefore take materially longer than one batched model call.

The endpoint must be `localhost` or a loopback IP; ComplyScan bypasses HTTP proxies and refuses redirects. Repository strings, code, and comments are explicitly labelled untrusted in the fixed prompts. Finding output must preserve submitted identifiers; unknown, changed, duplicate, or malformed bindings fail that review. Technical output contains only the decision fields, so the model cannot choose which candidate receives its response; malformed decisions still fail the requested review.
