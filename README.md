...
JSON and SARIF 2.1.0 output remain buffered so they are valid and deterministically ordered. Schema-version 11 JSON stdout and `latest.json` include applicability, the raw technical AI inventory, the `ai_use_inventory` overlay, per-use `ai_use_mappings`, technical evidence, reconciliation, the explicit `repository_analysis_run` lifecycle, and completed/total source-batch coverage. Version 10 introduced `repository_facts` and `role_candidates` under each confirmed, draft, retired, or suggested use plus report-level `ai_use_inventory.organization_unknowns`; version 11 retains those fields. It also retains version 9's optional `repository_analysis.result.objective_observations[].ai_use_id`, which is the supplied stable confirmed-use ID for a validated direct use observation and is omitted for a repository-level observation. SARIF includes source locations and stable partial fingerprints for code-scanning integrations, but omits technical-objective summaries that have no single code-scanning location.
