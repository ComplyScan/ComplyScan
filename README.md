...
JSON and SARIF 2.1.0 output remain buffered so they are valid and deterministically ordered. Schema-version 13 JSON stdout and `latest.json` include applicability, the raw technical AI inventory, the `ai_use_inventory` overlay, per-use `ai_use_mappings`, technical evidence, reconciliation, the explicit `repository_analysis_run` lifecycle, exact `member_observation_ids` for every inferred repository AI use, and the repository review's `provider_requests`, `source_batches_started`, `source_batches_completed`, and `source_batches_total` counters. `source_batches_completed` counts batches whose model output passed trusted validation; `source_batches_started` separately records source-bearing batches actually attempted. Version 12 introduced exact observation membership, version 11 introduced completed/total batch coverage, and version 10 introduced `repository_facts`, `role_candidates`, and report-level organisation unknowns. Version 9's optional direct confirmed-use observation binding remains available. SARIF includes source locations and stable partial fingerprints for code-scanning integrations, but omits technical-objective summaries that have no single code-scanning location.
...
When `ai.review-on-scan` is backed by matching private machine trust, or a provider is supplied with explicit one-run options, `complyscan scan` runs targeted repository AI reasoning after deterministic discovery, secret redaction, file classification, inventory, technical-evidence matching, and graph construction. Local code ranks AI signals, objective matches, production entry points, imports, callers, and nearby graph relationships, then prepares one bounded excerpt for every structurally selected candidate file. A small queue can fit one request; a larger queue is partitioned into provider-sized source batches and synthesized after every batch validates. The normal remote source-batch target remains about 6,500 input tokens, not per repository. Global synthesis contains validated summaries rather than raw source excerpts and may use the configured model input allowance, allowing those summaries to converge in one request when they fit instead of fanning out into many tiny synthesis calls. A single-package review may request one bounded follow-up with at most three literal searches; trusted code retrieves at most three new excerpts and, only when useful evidence is found, makes one final request. `--deterministic-only` never enters this path.

Provider responses may expose effective request-per-minute and token-per-minute capacity through rate-limit headers; there is no portable preflight endpoint that reports an API key's current tier. ComplyScan uses both request and token dimensions when both are available. A live source-free compatibility check can seed the first source wave. When compatibility was reused from the private cache and multiple source batches are needed, ComplyScan refreshes both bounded synthetic compatibility contracts before code transfer—normally two requests, with at most four attempts including temporary retries. Those requests are still metered, counted in `provider_requests`, and may incur provider cost. Complete capacity starts a header-bounded wave. A partial snapshot stays sequential. With no usable capacity metadata, hosted scheduling starts with one request and ramps conservatively after successful waves, up to the local concurrency ceiling of 32. Ollama stays serial because its local server does not advertise portable request/token capacity. Independent hosted source batches run concurrently, and after all source batches validate, independent groups within each synthesis level use the same scheduler; synthesis levels themselves remain ordered by their dependencies.
...
This adaptive request pattern is bound into the private consent fingerprint as context contract `adaptive-provider-pipeline-v2`. Machine approval created for an older transfer contract does not authorize it automatically; rerun `complyscan setup` to review the multiple-request and variable-cost disclosure, or use explicit one-run provider flags for that invocation.
