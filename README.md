...
The concise Markdown report is appended to the GitHub job summary by default, including when a required AI layer is incomplete; set `publish-summary: false` to disable it. Its selected-framework table names the mapped EU AI Act articles or voluntary-framework outcomes and distinguishes code signals found, no matching signal, and checks that could not be completed. Concrete controls and prioritized follow-ups retain their framework/provision label. These are repository-evidence states, not legal applicability, pass/fail, or compliance scores. The Action exposes absolute `markdown-report` and `json-report` output paths for an explicit later artifact step, but never uploads the raw JSON evidence bundle automatically. SARIF upload is independent.
