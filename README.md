...
In an interactive terminal, questions that accept several answers use a checkbox selector: move with ↑/↓, tick or untick with Space, and confirm with Enter. Redirected setup scripts keep the numbered comma-separated interface. Set `COMPLYSCAN_ACCESSIBLE=1` to force that stable text interface for screen readers or terminals where cursor-based controls are unsuitable.
