...
> Current release: v0.1.7. This README follows unreleased development on `main`, including the new `scan`/`review` command split. For behavior available in v0.1.7, use the documentation attached to that release.
...
complyscan review . # use the provider saved by setup
complyscan review . --provider openai --model gpt-5.6-sol --api-key-env OPENAI_API_KEY
complyscan ai-uses show # inspect the human-owned AI-use register
complyscan ai-uses setup # review suggestions from the latest completed AI review
...
First-run setup finishes by offering a local deterministic scan or saving configuration only. The first scan never invokes the configured model and reuses repository discovery already completed for onboarding. Setup can configure optional cloud or experimental local assistance for editable setup suggestions and later `complyscan review` runs. Remote setup explains external processing and possible cost, shows only shortlisted model IDs available to the account, and saves only the API-key environment-variable name.

`doctor` checks the installed build, repository configuration, Git detection, report-directory permissions, local Ollama readiness, the presence—not the value—of a configured remote credential, and any cached model-qualification status. Missing optional reviewer dependencies are warnings in an ordinary check and do not by themselves block the still-available deterministic scan. `complyscan doctor --probe-review` explicitly requests a fresh small synthetic structured-output and binding check, so a missing credential, service, or model becomes a blocking failure and the compatibility request is skipped. Remote probes may incur a small provider charge and never contain repository data.
...
`scan` defaults to the current directory, so `complyscan scan` and `complyscan scan .` are equivalent. It is guaranteed local and deterministic even when `.complyscan.yml` contains provider settings; it neither reads provider credentials nor makes a model request. Findings print as they are discovered. `complyscan review` runs the same deterministic foundation and then explicitly uses the configured provider, while `--provider` supplies a one-run override. The default completion is concise, and `--verbose` prints full framework and review detail. Use `unknown` rather than guessing.
...
The deterministic component inventory answers “which technical AI signals are present?” An explicit `complyscan review` may separately suggest product-level AI uses supported by code citations. Neither result silently becomes durable project state. Run `complyscan ai-uses setup` after a completed review to confirm a new use, merge a suggestion into an existing use, dismiss the unchanged suggestion with a reason, or decide later. The guided command lets the developer edit the name, description, repository paths, associated configured systems, and reviewer attribution. It is the only ComplyScan workflow that writes the register.
...
Both `complyscan scan` and `complyscan review` load the register locally, overlay current technical signals and optional model suggestions in the report, and never mutate the file. For every active developer-confirmed use, ComplyScan now filters technical-objective evidence to the use's saved paths and evaluates those objectives separately under each associated configured system. The system supplies declared applicability facts; the AI-use register supplies code scope. When a matching signal exists only outside the saved paths, the result records that separately so a developer can identify shared code or correct the scope instead of treating it as a simple absence. A missing or stale system association leaves legal and other context-dependent applicability unresolved; framework-wide voluntary practices can still remain recommendations. One use associated with several systems receives a separate result for each system context. Draft and retired uses remain visible but do not create current requirement mappings.

Use-scoped mappings are advisory and do not change deterministic findings or CI thresholds. An optional repository-review verdict is attached to a use only when every cited source location falls within that use's saved paths, its system attribution is compatible, and those citations identify exactly one confirmed use. Repository-wide negative conclusions without citations and observations that remain ambiguous across overlapping use scopes are not reassigned to an individual use. When two confirmed uses intentionally share code, both keep their deterministic mapping rather than being silently merged. If two uses need different questionnaire answers, associate them with different configured system profiles; sharing one system deliberately shares that declared context.

The exact manifest is excluded from repository discovery and model evidence, just like the active ComplyScan configuration. During `--changed-since` runs, deterministic signals are still collected repository-wide, but only changed and connected files enter the model review. A saved use outside that model scope may therefore show a current local signal while remaining unreviewed by the model; absence from the change can never delete or retire a human-owned entry. Confirming a repository grouping records developer knowledge, not legal applicability, safeguard effectiveness, or compliance.
...
Every completed scan or review publishes an immutable Markdown/JSON pair under `.complyscan/reports/history/<UTC date and time>/report.{md,json}`, then refreshes `.complyscan/reports/latest.md` and `latest.json`. History directories use a readable UTC name and are never overwritten with different content. The Markdown report is organized around what ComplyScan found, what to do next, and what it could not determine. A local scan labels deterministic matches as code signals. An explicit completed review can add one of four AI-assisted repository verdicts: implemented, partially implemented, not implemented in the reviewed code, or undetermined. These verdicts do not decide legal applicability, production enablement or operational effectiveness, and they do not change the deterministic CI threshold. Reviews save a complete preliminary report before their first model request and checkpoint completed layers, so provider failures cannot erase local results. Use `--require-ai-review` when automation should return exit code 2 for an incomplete explicitly requested review. Use `--no-report` to disable persistence or `--report-dir` to select another directory inside the target.

JSON and SARIF 2.1.0 output remain buffered so they are valid and deterministically ordered. Schema-version 8 JSON stdout and `latest.json` include applicability, the raw technical AI inventory, the `ai_use_inventory` overlay, per-use `ai_use_mappings`, technical evidence, reconciliation, and the explicit `repository_analysis_run` lifecycle. SARIF includes source locations and stable partial fingerprints for code-scanning integrations, but omits technical-objective summaries that have no single code-scanning location.
...
Explicit `complyscan review` runs targeted repository AI reasoning after deterministic discovery, secret redaction, file classification, inventory, technical-evidence matching, and graph construction. Local code ranks AI signals, objective matches, production entry points, imports, callers, and nearby graph relationships. The provider receives one compact package of selected source excerpts, manifests, configuration, CI or infrastructure evidence, the versioned code objectives, typed system facts, and report-safe graph context. The normal remote target is about 6,500 input tokens. The model may request one bounded follow-up with at most three literal searches; trusted code retrieves at most three new excerpts and, only when useful evidence is found, makes one final request. A plain scan never enters this path.
...
The targeted result performs both directions of the product workflow: it discovers technically evidenced AI uses in the selected structural context and maps evidence to supplied code objectives. AI activity that cannot be mapped remains visible with a reason. Every positive claim must cite a discovered path and valid line; invented paths, out-of-range lines, unknown objectives, unknown configured systems, invalid classifications, duplicate use IDs, and malformed output reject the pass. Unselected code is reported as not reviewed, never as absent. The model is not allowed to decide legal applicability, organisation role, deployment, real human practice, production effectiveness, or compliance when repository evidence cannot establish the narrower technical fact.

`bounded-only` retains the previous per-objective technical evidence investigation as a fallback and comparison mode. It can issue a planning and final decision request for each selected candidate or likely-required missing-evidence target, so it may create more calls than the targeted default. Neither mode's conclusions can change deterministic objective status, decide legal applicability, establish runtime effectiveness, or affect the finding-threshold exit code. Separately, `--require-ai-review` returns exit code `2` when an explicitly requested review layer is incomplete.
...
With `complyscan review --changed-since <git-ref>`, model source context is narrower as well: every changed eligible code or technical-configuration file plus at most eight unchanged files connected within two locally indexed graph hops. The same boundary constrains broad review modes, technical investigations, and model-requested follow-up. Unrelated unchanged source is never sent, while repository-wide governance checks and framework reconciliation continue locally. Reports distinguish the complete local scan from this `changed-plus-connected` model scope.
...
Interactive `complyscan setup` first inspects repository structure and technical signals locally without a model, then offers optional BYOK cloud assistance, model-free operation, and experimental local Ollama. After the privacy choice, an available configured model may receive bounded, best-effort-redacted repository context to draft editable setup answers. Saving those settings does not activate AI during `complyscan scan`; use `complyscan review` explicitly. Manual experimental Ollama review remains available:

```bash
ollama serve
ollama pull qwen3.5:9b
complyscan review . --provider ollama --ollama-model qwen3.5:9b
```

You can instead set `ai.provider: ollama` in `.complyscan.yml` and run `complyscan review`. If the provider is unavailable, the deterministic report is preserved and the review is marked incomplete. Add `--require-ai-review` when an incomplete review should return exit code `2`.
...
Remote repository analysis sends a locally selected, secret-redacted structural evidence package outside the machine. The same explicit review may send bounded secret-redacted finding records. Remote calls may incur cost and are governed by the provider account's processing and retention settings. ComplyScan sets `store: false` for native OpenAI and Gemini requests and refuses redirects. A custom compatible endpoint is an explicit additional trust boundary. These controls do not replace an organisational review of provider terms, endpoint ownership, region, retention, proxies, contracts, and permitted source-code processing.

Completed repository analysis and per-objective technical investigations are cached in the operating system's private user-cache directory, not in the scanned repository. Reuse requires the same active review-scope content, framework evidence, declared systems, ownership, context strategy, token budget, provider endpoint, model, and prompt contract. A full review binds the full discovered repository; `review --changed-since` binds its changed-plus-connected model scope while repository-wide framework evidence remains part of the identity. Cache files store validated model results plus cryptographic input digests, never the submitted source-context records or credentials. Model rationales and citations may still describe repository details and should be treated as potentially sensitive. A cache hit is identified in terminal, Markdown, and JSON output and makes no repository-analysis model call. Use `complyscan review --refresh-review` to bypass both review caches and obtain fresh inference.
...
ComplyScan uses separate repository and finding review flows. Repository `auto` and `targeted` modes locally construct a compact structural evidence package and use one model call, plus at most one final call after a useful bounded follow-up. `deep` retains automatic broad full-or-hierarchical analysis, `full` requires one broad request, `hierarchical` always partitions and synthesizes, and `bounded-only` disables repository-level reasoning while retaining the older per-objective technical investigation. Finding review is separately bounded by `max-findings` and sends visible, unsuppressed deterministic finding records with re-redacted metadata such as rule ID, fingerprint, title, message, relative path, line, short evidence, remediation, severity, and confidence. Submitted excerpts are re-redacted and are not copied into the saved report.
...
Finding observations are attached to existing fingerprints as `confirmed`, `uncertain`, or `not_supported`. Technical investigations remain attached to the exact system, framework, objective, and evidence target. Candidate validation and missing-evidence search use only code assigned or shared with that system; conflicting and unassigned paths are excluded. Repository observations remain a separate advisory result and do not silently replace the configured system profile, human-owned AI-use register, or deterministic reconciliation. Use-level reports accept a repository observation only when its checked citations are entirely inside the confirmed use scope. All review types are advisory: they cannot create or remove findings, change objective status or severity, write AI-use groupings, update baselines or suppressions, or affect the failure threshold. Terminal, Markdown, and schema-version 8 JSON expose repository-analysis coverage and follow-up metadata; SARIF carries finding review only. ComplyScan requests non-streaming schema-constrained output; Ollama additionally receives temperature zero, while modern remote models use their provider defaults where sampling parameters are unsupported or deprecated. Model output can still vary.
...
The v0.1.7 example above performs deterministic scanning. That release can opt into review with a supported provider name and accepts the existing `ollama-model`, `ollama-endpoint`, `model`, and `api-key-env` inputs. On unreleased `main`, the Action makes the boundary more explicit: empty or `none` remains deterministic, `review: configured` uses the saved provider, and a provider name is a one-run override. Unreleased `main` also adds `require-ai-review`, `provider-name`, `base-url`, and the expanded compatible-provider set. Those new semantics and inputs require the next release and are not available through the v0.1.7 tag. For BYOK review, place the credential value in a GitHub Actions secret exposed under the configured environment-variable name—never in an action input or committed YAML.

```yaml
- uses: ComplyScan/ComplyScan@<release-tag-with-explicit-review>
  env:
...
`complyscan review` is a separate explicit processing boundary. It may send the selected best-effort-redacted structural package and bounded finding records to the configured provider and may incur cost. Ollama requests use a validated loopback endpoint; hosted requests use the selected HTTPS endpoint after setup disclosure and consent. The active ComplyScan configuration is excluded from model context, although separately constructed typed system facts may be supplied. Submitted source excerpts are not stored in reports or the OS user-cache, but model rationales may describe repository details. Generated reports remain local unless a future dashboard connection is explicitly enabled.
