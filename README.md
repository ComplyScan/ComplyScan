...
Targeted repository AI reasoning runs after deterministic discovery, secret redaction, file classification, inventory, technical-evidence matching, and graph construction. Local code ranks AI signals, objective matches, production entry points, imports, callers, and nearby graph relationships. The provider receives one compact package of selected source excerpts, manifests, configuration, CI or infrastructure evidence, the versioned code objectives, typed system facts, and report-safe graph context. The normal remote target is about 6,500 input tokens and 3,000 output tokens. The model may request one bounded follow-up with at most three literal searches; trusted code retrieves at most three new excerpts and, only when useful evidence is found, makes one final request. The default path therefore uses one or at most two repository-analysis calls rather than one request per directory or objective.
