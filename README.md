...
Targeted repository AI reasoning runs after deterministic discovery, secret redaction, file classification, inventory, technical-evidence matching, and graph construction. Local code ranks AI signals, objective matches, production entry points, imports, callers, and nearby graph relationships. The provider receives one compact package of selected source excerpts, manifests, configuration, CI or infrastructure evidence, the versioned code objectives, typed system facts, and report-safe graph context. The normal remote target is about 6,500 input tokens and 4,096 output tokens. OpenAI GPT-5.6 requests explicitly use low reasoning effort and low text verbosity, and the targeted schema caps item counts, citations, questions, and text length. The model may request one bounded follow-up with at most three literal searches; trusted code retrieves at most three new excerpts and, only when useful evidence is found, makes one final request. If the initial response instead exhausts its output allowance, that same sole second-call budget is used for a terse no-follow-up recovery with reasoning disabled. The default path therefore uses one or at most two repository-analysis calls rather than one request per directory or objective.
