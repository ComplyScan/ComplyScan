...
ComplyScan is an open-source, offline-capable CLI for finding technical signals that deserve review against selected AI governance sources. Guided setup records factual system context, lets the operator select technical evidence packs, and collects attributable human applicability decisions where legislation requires them. A framework-neutral control layer maps code and configuration signals to both EU AI Act technical objectives and voluntary NIST AI RMF practices. Scans also inventory likely AI providers and frameworks, look for risky logging and hardcoded credentials, and check whether repository-level AI-system and risk-classification evidence is present. The recommended model-assisted path uses a small OpenAI, Anthropic, or Gemini BYOK shortlist. After explicit consent, local structural analysis selects a compact, redacted evidence package for cross-file model reasoning and permits at most one bounded follow-up. Broad whole-repository transfer remains an explicit deep mode rather than the default. Fully model-free scanning remains available; Ollama is retained as an advanced experimental path rather than presented as equivalent to frontier cloud review.
...
`scan` defaults to the current directory, so `complyscan scan` and `complyscan scan .` are equivalent. Every scan runs the deterministic layer; when setup configured a reviewer, the same command automatically adds targeted repository AI reasoning and bounded finding review. Passing an explicit one-off `--review none` disables model review for that run, while another explicit `--review` overrides the configured provider. Findings still print as they are discovered. The default completion is concise, while `--verbose` prints every framework objective and model observation in the terminal. Use `unknown` rather than guessing. Redirected or CI initialization is non-interactive and can be made explicit with `--non-interactive`.
...
Every completed scan publishes an immutable Markdown/JSON pair under `.complyscan/reports/history/<UTC timestamp>-<scan ID>/report.{md,json}`, then atomically refreshes the backward-compatible `.complyscan/reports/latest.md` and `latest.json` snapshots. Historical pairs are staged together and exposed through one directory rename, are never overwritten with different content, and are not deleted automatically. During AI review, recovery checkpoints update only `latest.*`; one history bundle is created after the final report is complete. The Markdown report is a short, plain-language developer page organized around three questions: what ComplyScan found, what to do next, and what ComplyScan could not determine. When AI code review completes, trusted code converts each validated observation into one of four explicit repository verdicts: implemented, partially implemented, not implemented in the reviewed code, or undetermined. A strong/high-confidence decision requires cited implementation evidence with no cited contradiction or missing objective element; weaker, incomplete, or unsupported observations cannot become “implemented.” The report shows that decision and its short rationale instead of falling back to “possible implementation; a person must check.” Without a completed model observation, deterministic matches remain clearly labelled as unverified code matches. Raw confidence scores, exhaustive requirement rows, internal IDs, evidence strengths, fingerprints, graph relationships, and ownership diagnostics remain in the JSON bundle. Legal and technical source references are collapsed by default. AI libraries and configuration are kept separate from AI features identified from repository context, and only likely-required gaps or other actionable mappings appear in the next-step list. Code-level verdicts do not decide legal applicability, production enablement or operational effectiveness, and they do not change CI exit status. The human report distinguishes a model review that was not requested, one that has started, one that completed, and one that was attempted but incomplete. A configured AI scan writes the complete preliminary report before its first model request and checkpoints completed review layers, so provider failures cannot erase deterministic results. Invalid model output is recorded as incomplete while deterministic and bounded evidence remain available. The schema-version 6 bundle adds `repository_analysis` with its context mode, coverage, verified citations, discovered AI uses, technical-objective observations, derived technical verdicts, unmapped activity, unresolved questions, and usage. Its `frameworks` array retains each pack's identity, nature, applicability where relevant, technical evidence, reconciliation, and optional grounded investigation. Transitional EU-only top-level fields remain populated for compatibility and will be removed only in a future breaking schema version. `complyscan init` adds the generated directory to `.gitignore`; scans always exclude it from subsequent discovery. Use `--no-report` to disable persistence or `--report-dir` to select another directory inside the target.
...
Reports saved:
  ...
  Historical human-readable: .complyscan/reports/history/20260814T142530Z-scan-a81f/report.md
  Historical evidence bundle: .complyscan/reports/history/20260814T142530Z-scan-a81f/report.json
```
...
Targeted repository AI reasoning runs after deterministic discovery, secret redaction, file classification, inventory, technical-evidence matching, and graph construction. Local code ranks AI signals, objective matches, production entry points, imports, callers, and nearby graph relationships. The provider receives one compact package of selected source excerpts, manifests, configuration, CI or infrastructure evidence, the versioned code objectives, typed system facts, and report-safe graph context. The normal remote target is about 6,500 input tokens. OpenAI GPT-5.6 requests use medium reasoning effort and low text verbosity, and the targeted schema caps item counts, citations, questions, and text length. The first request uses a conservative 4,096-token output allowance because an account's token-per-minute ceiling is not known before the provider responds. If that allowance is exhausted, ComplyScan reads OpenAI's returned account/project token ceiling and uses the sole second call for a terse no-follow-up response with medium reasoning and the maximum usable output allowance: the smaller of the model ceiling and the provider ceiling minus the actual input tokens. If the header is unavailable, the recovery retains the conservative allowance. The model may otherwise request one bounded follow-up with at most three literal searches; trusted code retrieves at most three new excerpts and, only when useful evidence is found, makes one final request. The default path therefore uses one or at most two repository-analysis calls rather than one request per directory or objective.
...
When model review is configured, ComplyScan sends the targeted redacted structural package plus bounded finding records described above. Explicit `deep`, `full`, and `hierarchical` modes send substantially more eligible repository content; model-free analysis sends no source externally. Ollama requests use a validated loopback endpoint; hosted requests use the selected native, preset, or explicitly configured HTTPS endpoint after setup disclosure and consent. The raw active ComplyScan configuration is excluded before inventory, rules, graph construction, repository analysis, and evidence selection; derived typed system facts needed to interpret technical objectives may still enter analysis. Submitted source is not stored in the evidence bundle or OS user-cache. Model rationales, questions, and suggested actions may describe repository details; they are re-redacted and length-limited before reporting or caching. Credential values do not enter model context. Generated reports and cached observations remain local unless a future dashboard connection is explicitly enabled.
