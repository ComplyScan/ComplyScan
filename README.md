...
This adaptive request pattern is bound into the private consent fingerprint as context contract `adaptive-provider-pipeline-v3`. Machine approval created for an older transfer contract does not authorize it automatically; rerun `complyscan setup` to review the multiple-request and variable-cost disclosure, or use explicit one-run provider flags for that invocation.
