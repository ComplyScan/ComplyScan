...
ComplyScan is an open-source, offline-capable CLI for finding technical signals that deserve review against selected AI governance sources. Ordinary guided setup asks only for the model/provider and technical evidence packs; the scan infers code-visible facts, while organisation-only unknowns stay in the report for a future compliance-owner workflow. A framework-neutral control layer maps code and configuration signals to both EU AI Act technical objectives and voluntary NIST AI RMF practices. `complyscan scan` is the single normal workflow: it always runs the local deterministic checks and requires bounded advisory AI reasoning from an approved provider. Local structural analysis selects best-effort-redacted candidate excerpts for OpenAI, Anthropic, Gemini, or experimental Ollama; if no structural AI anchor is found, a small representative presence check prevents a purely local selector from silently deciding that no model review is needed. Small queues fit one package, while larger queues use multiple bounded source requests plus synthesis. Use `complyscan scan --deterministic-only` for the explicit offline workflow that never uses a model or provider credential.
...
For a concise explanation of the product, user workflow, EU AI Act technical mappings, and optional voluntary NIST AI RMF mapping, see the [two-page ComplyScan overview](docs/complyscan-overview.pdf).
...
complyscan --deterministic-only # scan the current repository without setup or a model
...
complyscan actions list # show active evidence-backed developer work
complyscan actions show <action-id> # inspect evidence and acceptance criteria
complyscan actions verify <action-id> # rescan and check whether it is resolved
complyscan agent instructions --format skill # print coding-agent instructions
complyscan agent serve # expose read-only ComplyScan MCP tools over stdio
...
First-run setup finishes by offering to run ComplyScan or save configuration only. If the user selected an AI provider after the privacy and cost disclosure, setup saves repository intent as `ai.review-on-scan: true` and a matching private machine-local trust record; the same-machine standard scan then runs deterministic checks plus the configured advisory review. If no provider was selected, setup shows `complyscan scan --deterministic-only` as the next command. Remote setup shows only shortlisted model IDs available to the account and saves only the API-key environment-variable name in repository configuration.
...
### Developer actions and coding agents

Every schema-version 17 evidence bundle contains a structured `developer_actions` list derived locally from checked findings, framework mappings, repository observations, execution checks, and incomplete scan states. Each action has a stable ID, status, priority, plain-language reason and change, acceptance criteria, cited evidence, framework/provision mapping, and verification method. Comparing a completed scan with the previous local bundle moves an item through `new`, `open`, `resolved`, or `reopened`; partial and change-scoped scans do not falsely resolve work they did not cover. Dashboard-owned decisions may retain `accepted` status without changing scanner evidence.

Use `complyscan actions list`, `actions show`, and `actions verify` to move from a report to one bounded engineering task. `complyscan agent instructions` prints a concise `AGENTS.md` fragment or, with `--format skill`, a reusable coding-agent skill; it writes only when `--write` is explicit. `complyscan agent serve` exposes read-only MCP stdio tools for listing actions, retrieving evidence, scanning changed code deterministically, and rechecking deterministic findings. The MCP tools never activate an AI provider or write report artifacts. Human-only product and organisation questions remain handoffs rather than fabricated code fixes.
...
This adaptive request pattern, including the bounded representative presence check used when no structural AI anchor is found, is bound into the private consent fingerprint as context contract `adaptive-provider-pipeline-v6`. Machine approval created for an older transfer contract does not authorize it automatically; rerun `complyscan setup` to review the multiple-request and variable-cost disclosure, or use explicit one-run provider flags for that invocation.
...
Finding observations are attached to existing fingerprints as `confirmed`, `uncertain`, or `not_supported`. Technical investigations remain attached to the exact system, framework, objective, and evidence target. Candidate validation and missing-evidence search use only code assigned or shared with that system; conflicting and unassigned paths are excluded. Repository observations and per-use repository facts remain a separate advisory overlay and do not silently replace the configured system profile, human-owned AI-use register, or deterministic reconciliation. Compact source models return atomic observations without inventing candidate IDs; each citation uses a trusted request-local block ID and original line, and ComplyScan assigns scan-local observation identity after validation. Global synthesis decides only observation membership. All review types remain advisory and cannot change deterministic findings or CI gates. Terminal and schema-version 17 JSON expose detailed repository-analysis coverage, grouping status, source batches started versus validated, total source batches, provider-request count, and follow-up metadata. The default developer Markdown keeps only a concise assessment boundary and prominent incomplete-coverage warnings; check-by-check and provider accounting stays in JSON for dashboard and audit use. Version 17 also adds stable developer actions and repository/configuration/framework-pack provenance; version 16 added grouping and per-attempt token diagnostics. JSON additionally records checked cross-batch gap resolutions and one privacy-bounded duration/outcome/retry/token diagnostic per repository-layer provider attempt for dashboard audit; these diagnostics contain no prompt, source content, file list, response body, or provider request ID. If every source batch validates but grouping fails, the report retains each observation separately and marks only AI-use organization incomplete; an incomplete source review retains accounting and coverage but no unvalidated conclusions. ComplyScan requests non-streaming schema-constrained output and fixes provider controls by task: source extraction, finding review, setup inference, and search planning use low reasoning effort; grouping synthesis and technical judgments use medium. OpenAI and Anthropic retain their reasoning-model sampling defaults, Gemini uses a fixed seed, and Ollama uses temperature zero. Model output can still vary.
...
The Action always invokes the unified `scan` command, but its first-class `ai-review` input defaults to `none` and explicitly passes deterministic-only operation even when the repository contains `ai.review-on-scan: true`. This safe CI default is separate from the local CLI's standard AI-required contract. To permit source processing in CI, set `ai-review: configured` explicitly or, preferably, pin a provider and model with workflow inputs as shown below. `configured` deliberately trusts the provider destination and credential environment-variable name committed in `.complyscan.yml`. A missing key or unavailable model still leaves valid deterministic SARIF and reports but makes the requested AI run incomplete. With `ai-review: none`, `require-ai-review: true` still runs the deterministic scan and saves reports before returning exit code `2`; it never grants processing consent. AI verdicts remain advisory and never change the deterministic finding threshold.

The concise Markdown report is appended to the GitHub job summary by default, including when a required AI layer is incomplete; set `publish-summary: false` to disable it. For pull requests, the Action runs one additional model-free scan of the checked-out base revision and exposes new, reopened, resolved, and active action counts. Located new or reopened code-control actions also appear as SARIF annotations with their stable action ID and acceptance criteria. After the prioritized developer actions, one framework technical-assessment section names the mapped EU AI Act articles or voluntary-framework outcomes, distinguishes code signals found, no matching signal, and checks that could not be completed, then gives each key control's code-level conclusion, evidence, and next step. These are repository-evidence states, not legal applicability, pass/fail, or compliance scores. The Action exposes absolute `markdown-report` and `json-report` output paths plus `active-actions`, `new-actions`, `reopened-actions`, and `resolved-actions`; it never uploads the raw JSON evidence bundle automatically. SARIF upload is independent.

The recommended pipeline keeps pull requests fast and deterministic: use the default `ai-review: none`, let `scope: auto` review changed and locally connected code, and make that check required for merging. Run source-bearing AI review separately from a trusted branch through a manual, scheduled, or release workflow with protected provider credentials. This repository follows that split: `.github/workflows/complyscan.yml` is the required deterministic self-scan, while `.github/workflows/ai-review.yml` is an explicitly dispatched full-repository advisory review. Fork pull requests still receive the deterministic report but skip SARIF upload because their GitHub token cannot normally receive write permission.
...
When guided setup records both repository intent and matching private machine trust, later same-machine scans may send one or more best-effort-redacted structural candidate packages, a bounded representative presence package when no structural anchor exists, synthesis context, and bounded finding records to that configured provider and may incur variable cost. Metered compatibility checks, validation repairs, and transient or rate-limit retries can add requests; source-bearing retries resend the bounded package. Cached compatibility is not repeated only to obtain capacity headers. Concurrency may consume the same variable quota and cost more quickly, while finite retry and request ceilings mean permanent provider, quota, or schema failures can still leave the AI layer incomplete. A clone receives repository intent but not private trust, so a standard scan fails with setup guidance until the user approves it or supplies explicit one-run provider options; deterministic-only remains available. Ollama requests use a validated loopback endpoint; hosted requests use the selected HTTPS endpoint after setup disclosure and consent. The active ComplyScan configuration is excluded from model context, although separately constructed typed system facts may be supplied. Submitted source excerpts are not stored in reports or the OS user-cache, but model rationales may describe repository details. Provider failures preserve deterministic results and make standard scans exit incomplete. CLI reports remain local unless an explicit integration publishes them; the GitHub Action behavior is described below, and no dashboard connection exists yet.
