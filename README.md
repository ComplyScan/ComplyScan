...
Technical investigations are cached in the operating system's private user-cache directory, not in the scanned repository. The cache stores bounded observations plus hashes of submitted context; it does not store submitted source excerpts. Model rationales may still describe repository details and should be treated as potentially sensitive. Use `complyscan review --refresh-review` to bypass cached technical observations. Targeted repository analysis itself is not cached yet.
