...
> Current release: v0.1.4. Model review is optional and experimental; deterministic scanning remains the default.
...
```bash
curl -fsSL https://github.com/ComplyScan/ComplyScan/releases/download/v0.1.4/install.sh | sh -s -- --version v0.1.4 --no-setup
```
...
```bash
go build -ldflags "-X main.version=0.1.4 -X main.commit=$(git rev-parse --short HEAD) -X main.buildDate=$(date -u +%Y-%m-%dT%H:%M:%SZ)" -o complyscan ./cmd/complyscan
```
...
steps:
  - uses: actions/checkout@v6
    with:
      fetch-depth: 0
  - uses: ComplyScan/ComplyScan@v0.1.4
    with:
      severity: medium
...
In the default deterministic mode, the ComplyScan v0.1.4 CLI:
