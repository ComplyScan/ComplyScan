...
`complyscan scan` always runs deterministic repository rules, inventory, graph construction, technical-objective matching, reconciliation, reporting, and configured failure thresholds, and a standard run requires semantic evidence investigation from an approved model. Guided setup records both `ai.review-on-scan: true` repository intent and matching private trust on that machine after a user selects the model boundary. A clone or ephemeral runner has no transferable trust, so a standard scan fails with approval guidance unless setup runs locally or provider options supply explicit one-run consent. `complyscan scan --deterministic-only` is the explicit workflow available without a model, API key, or external processing. A provider failure after discovery preserves deterministic results but makes the standard scan incomplete; `--require-ai-review` remains a compatibility and CI policy flag rather than consent, and model verdicts never alter deterministic finding thresholds.
