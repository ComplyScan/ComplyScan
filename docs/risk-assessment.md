...
| R-20 | Keyword matching treats unrelated code as candidate evidence, misses differently implemented controls, or encourages checklist compliance. | High | High | Matches are labelled candidates; grouped terms and eligible technical file kinds reduce noise; stable paths and lines make review possible; Go graph context exposes reachability and connected controls; optional semantic review remains separate; statuses stop at `candidate-evidence`; unsupported source is `not-evaluated`; and technical results do not gate CI. Broader labelled technical-evidence and live-model evaluation are still required. | Medium |
