...
| Status | Updated for optional Ollama, code-only technical packs, and automatic local reports; live-model validation and applicability review remain open before release |
...
| R-20 | Keyword matching treats unrelated code as candidate evidence, misses differently implemented controls, or encourages checklist compliance. | High | High | Matches are labelled candidates; grouped terms and eligible technical file kinds reduce noise; stable paths and lines make review possible; statuses stop at `candidate-evidence`; no unmatched check proves absence; technical results do not gate CI. Labelled technical-evidence evaluation and connected semantic review are still required. | Medium |
