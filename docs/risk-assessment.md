...
| R-20 | Keyword matching treats unrelated code as candidate evidence, misses differently implemented controls, or encourages checklist compliance. | High | High | Matches are labelled candidates; grouped terms and eligible technical file kinds reduce noise; stable paths and lines make review possible; Go and Python graph context exposes reachability and connected controls; optional semantic review remains separate; statuses stop at `candidate-evidence`; unsupported source is `not-evaluated`; and technical results do not gate CI. Broader labelled technical-evidence and live-model evaluation are still required. | Medium |
...
ComplyScan 0.1.2 combines self-declared profile facts with repository evidence rather than observing a complete deployed system. It cannot verify that profile answers match real operations, runtime configuration, actual data subjects, organisational controls, intended use outside the repository, or downstream decisions. Ollama sees only bounded findings or connected technical context and therefore cannot resolve most missing system context. The relationship graph currently indexes Go and conservatively structured Python; dynamic dispatch, runtime framework registration, JavaScript, TypeScript, and other languages remain incomplete coverage. These limitations are communicated to users and are not treated as defects that can be eliminated solely through more rules or model prompts.
...
- Go and Python graph and adversarial repository fixtures covering imports, calls, routes, feature configuration, authorization, persistence, audit logging, live reachability, test-only dead code, unsupported source, and source-comment prompt injection;
...
The first live `qwen3:8b` smoke test passed on 2026-08-04: the production-routed candidate was `partial`, both test-only candidates were `weak`, the prompt-injection fixture remained bounded, and no deterministic model-output correction was required. Timing and memory observations are recorded in [ollama-validation.md](ollama-validation.md). This closes the small adversarial-fixture gate only. Before Ollama review is promoted from experimental status, verification must also include larger representative repositories, supported-language coverage, repeated-run stability, and documented acceptance thresholds. Automated tests do not download or execute an Ollama model, so maintainers must rerun `./scripts/validate-ollama.sh` for every proposed model or material prompt/context change and retain its generated summary for review.
