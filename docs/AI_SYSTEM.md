...
ComplyScan is an offline-by-default developer CLI that identifies technical signals and missing repository evidence relevant to AI compliance engineering. It helps developers find items that require human review; it does not determine legal compliance, assign a legally binding risk classification, or replace qualified legal and compliance review. Users may explicitly enable an advisory review layer through local Ollama, a native BYOK provider, or a supported OpenAI-compatible API for already-sanitised deterministic findings and bounded connected technical-objective context.
...
Model review is disabled by the built-in scan default. Interactive setup presents the local, hosted, and model-free privacy choices first; Ollama installation or model pulls require separate confirmation. Selecting any hosted provider triggers a separate disclosure and default-no external-processing confirmation; automation requires `--allow-remote-review`. The default scan remains deterministic and makes no model call. Ollama now recommends `qwen3.5:9b` as its default candidate; `qwen3:8b` retains the recorded experimental validation baseline. The prompt-version 10 multi-framework fixture gate passed across Go, Python, and TypeScript with `qwen3:8b`, separate EU and NIST objective bindings, no semantic corrections, and bounded prompt-injection content. After discarding a sleeping-host measurement, a controlled awake TypeScript rerun completed in 592.9 seconds on 2026-08-08. Those results do not validate `qwen3.5:9b`, which must pass the same live gate. This remains a small quality-contract and timing fixture rather than a general performance claim. BYOK adapters have fake-transport contract tests but no maintained live-model quality baseline yet; they remain experimental. ComplyScan Cloud remains an inactive extension type.
...
The setup questionnaire and human-confirmed profile remain local. After each completed major stage, terminal setup may save the configuration answers in a repository-keyed recovery draft under the operating system's user-cache directory. The draft contains no repository source or API-key value, is size-bounded, atomically written with user-only permissions, refuses symlinks, expires after 30 days, and is deleted after `.complyscan.yml` is saved. The final review still requires the user to save, revisit, or cancel the collected configuration. After separate explicit confirmations, setup may invoke Homebrew, download and execute Ollama's official Linux installer, start the Homebrew Ollama service, or invoke `ollama pull` for the selected model. Profiles and repository source are not passed to those provisioning commands. Before an interactive setup or deep scan trusts an unseen model with repository context, ComplyScan sends one source-free synthetic finding and requires a correctly bound schema-valid response. Successful compatibility results are cached outside the repository for 30 days against provider, exact model, available Ollama digest, and every prompt-contract version; failures are not cached. Non-interactive setup makes this request only with `--qualify-model`. The request may incur remote-provider cost but contains no repository data. Compatibility demonstrates only minimum protocol behavior, not review quality or benchmark validation. When a selected reviewer is ready, setup may make one profile-draft request containing at most 24 secret-redacted repository contexts of at most 2,500 characters each; it does not send human answers or the resulting profile. Every returned suggestion shows confidence, rationale, and repository citations and remains editable before confirmation. Jurisdiction, organisation role, actual production status, legal applicability, legal risk class, and unsupported negative data claims are not inferred. Remote setup saves only provider, model, limits, and an environment-variable name after disclosure and consent. The credential value remains in the shell or CI secret store. `doctor --probe-review` refreshes the same synthetic compatibility result and may incur cost.
