...
Completed repository-level analysis has a separate private cache with the same no-source-excerpt rule. Its identity additionally binds the provider endpoint, repository content, framework evidence, declared profiles, ownership rules, context-selection version and strategy, and token budget. Cache hits are explicitly reported and clear current-run token usage because no repository-analysis request was made. Model summaries and citations remain potentially sensitive even without submitted excerpts, and `--refresh-review` bypasses this cache as well.
