...
That record validates the earlier candidate-review prompt, not the current prompt-version-9 system-scoped evidence-investigation, bounded-retrieval, and isolated-test interpretation schema. A prompt-version-8 two-target smoke fixture passed the grounded-claim, assurance, retrieval, and malformed-plan fallback contract on 2026-08-06, but neither it nor the historical 17-candidate figures validate version 9 until the live benchmark is rerun.
