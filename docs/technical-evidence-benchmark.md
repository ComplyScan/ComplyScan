...
With Ollama running and the model named in `testdata/technical-evaluation/external/semantic.json` installed—currently `qwen3.5:9b`—use the evaluator's `--review ollama` option to evaluate every deterministic candidate. The evaluator invokes ComplyScan's explicit review path. The dated results below remain narrow historical evidence; live-model evaluation is manual, slow, variable, and cannot support a general accuracy claim.
