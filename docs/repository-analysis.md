...
The normal trusted or explicitly requested AI path therefore uses one model call, or at most two when a useful follow-up is requested. It does not create one request per directory, objective, or confirmed AI use; the structured response still keeps every direct use/objective/system evaluation separate. If trust, the model, key, or service is unavailable, ComplyScan preserves the deterministic report and records or explains the absent or incomplete AI layer. `--require-ai-review` can make that state return exit code `2` after report artifacts are written, but never grants consent by itself.
...
A configured AI layer records suggestions and observations in schema-version 9 reports but never writes the register. Version 9 adds the optional `repository_analysis.result.objective_observations[].ai_use_id` for a direct observation keyed to a supplied stable use; repository-level observations omit it. The report remains useful without a register. Developers who want more precise per-use mappings can optionally run `complyscan ai-uses setup` to confirm a new grouping, merge it into an existing grouping, dismiss the unchanged suggestion, or decide later. `complyscan ai-uses edit` updates an existing saved grouping without scanning or contacting a model; it preserves the stable ID and existing suggestion links. Stable IDs are generated locally, so a different model or future run cannot silently rename durable uses.
...
- Completed repository results are reused only for an exact private-cache identity covering active review-scope content, framework evidence, profiles, ownership, derived confirmed-use IDs/path scopes/system-objective bindings, provider endpoint, model, prompt contract, context strategy, and token budget. The deterministic selector derives submitted files from those inputs. A full scan binds the full discovered repository; `scan --changed-since` binds the changed-plus-connected model scope defined above. Any changed identity input causes a new request; `--refresh-review` always bypasses reuse.
